...
naive_flatten_tree(nil,Flattened):-
Flattened = [].
naive_flatten_tree(leaf(A), Flattened):-
Flattened = [A].
naive_flatten_tree(tree(X,Y,Z), Flattened):-
naive_flatten_tree(X,F1),
naive_flatten_tree(Z,F2),
append(F1,[Y|F2],Flattened).	
...
flatten_tree(nil,Flattened):-
Flattened = [].
flatten_tree(leaf(A), Flattened):-
Flattened = [A].
flatten_tree(tree(X,Y,Z), Flattened):-
flatten_tree(X,F1),
flatten_tree(Z,F2),
reverse(F1,Flattened, [Y|F2]).	
